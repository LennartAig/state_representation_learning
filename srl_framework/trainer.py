...
from srl_framework.utils.replay_buffer import Buffer, ReplayBuffer, SequentialReplayBuffer
...
class Trainer():
    ...
    def __init__(self, args):
        ...
        if self.srl_rl_initial_update_steps > 0: self.srl_initial_update_steps = 0
        ...
        # Environment
        curl = ('LATENT' in self.param.SRL.MODELS and self.param.SRL.LATENT.TYPE =='CURL')
        image_size = args.render_image_size if curl else args.input_image_size
        self.env, self.envtype, self.obs_shape, self.act_dim, self.act_limit = make_env(args,image_size)
        try:
        ...
        kwargs = {
            ...
            'device':self.device,
            'obs_shape': (self.obs_shape[0],image_size,image_size) if curl else self.obs_shape,
        ...
        print('--------------------RL AGENT: {} ---------------------'.format(self.param.RL.NAME))
        ...
        if self.param.RL.NAME == 'sac':
            rl_kwargs = {
                ...
                'target_encoder': self.srl_module.latent_model.encoder_targ if self.srl_info['contrastive'] else None
            }
            self.batchsize = self.param.RL.BATCH_SIZE
            if self.srl_info['contrastive']: self.srl_module.do_target_update = True
            ...
            self.batchsize = self.steps_per_epoch
            self.target_kl = self.param.RL.PPO.MAX_KL_DIV if self.param.RL.PPO.EARLY_STOPPING else None
        else:
        ...
        encoder=self.srl_module.latent_model.encoder if self.srl_info['latent'] else None
        ...
        if self.agent_class == 'on_policy':
            self.memory = Buffer(capacity= self.buffer_size, obs_shape=self.env.observation_space.shape,
                act_dim=self.act_dim, device=self.device, **buffer_kwargs)
        elif self.agent_class == 'off_policy' and not self.srl_info['sequential']:
            self.memory = ReplayBuffer(capacity=self.buffer_size, obs_shape=self.env.observation_space.shape,
                act_dim=self.act_dim, device=self.device, **buffer_kwargs)
        elif self.agent_class == 'off_policy' and self.srl_info['sequential']:
            self.memory = SequentialReplayBuffer(
                capacity = self.buffer_size, obs_shape = self.env.observation_space.shape,
                act_dim = self.act_dim, device = self.device, **buffer_kwargs)
        else:
            raise NotImplementedError
    ...
    def step(self, obs, steps=-1, deterministic = False):
        ...
        if steps > self.rl_delayed_start or steps == -1: # Action sampled by Policy
            ...
            if self.srl_info['state_type'] == 'feature_hidden':
                self.srl_module.state_space_model.infer(features, torch.FloatTensor(act).to(self.device))
            ...
            if self.agent_class == 'on_policy':
                with torch.no_grad():
                    state, features = self.srl_module.get_state(obs)
                    _, log_prob = self.rl_agent.actor(state,torch.FloatTensor(act).to(self.device))
        ...
        if self.agent_class == 'on_policy' and self.sim_env== 'dmc' and self.envtype=='Box':
        ...
        for epoch in range(self.num_epochs):
            ...
            for t in range(self.steps_per_epoch):
                ...
                if done or (ep_len == (self.max_eplen)):
                    ...
                    if self.srl_info['state_type'] == 'feature_action_seq': 
                        obs_deque, action_deque = self.reset_deque(obs, True)
                        if not done: self.memory.finish_path()
                    obs, ep_ret, ep_len = self.env.reset(), 0, 0
                
            if self.agent_class == 'off_policy'and self.srl_info['sequential'] and not done: self.memory.finish_path()       
            self.logger.store(EpRet=ep_ret, EpLen=ep_len)
            if self.agent_class == 'on_policy': self.optimize_on_policy(total_steps, epoch)
            
            # Just hack to use logger
            if total_steps <= self.initial_srl_steps: self.logger.store(**self.srl_loss_info)
            if total_steps <= self.initial_rl_steps: self.logger.store(**self.rl_info)
            ...
            for key, _ in self.srl_loss_info.items():
                self.logger.log_tabular(key, epoch = epoch, with_min_and_max=False, tensorboard= True)
            for key, _ in self.rl_info.items():
                self.logger.log_tabular(key, epoch = epoch, with_min_and_max=False, tensorboard= True)
            
            self.test_agent(epoch)
            self.logger.log_tabular('TestEpRet', epoch = epoch, with_min_and_max=True, tensorboard= True)
            self.logger.log_tabular('TestEpLen', epoch = epoch, average_only=True, tensorboard= True)
    ...
    def test_agent(self, epoch=0):
        ...
        test_env = self.env
        if log_video and epoch%self.video_freq==0: self.logger.video_rec.init(enabled=True)
        for test_ep in range(self.test_episodes):
            ...
            while not(done or (ep_len == self.max_eplen)):
                ...
                if log_video and epoch%self.video_freq==0: self.logger.video_rec.record(test_env, camera_id=8)
                ep_ret += reward
                ep_len += 1
                if self.srl_info['state_type'] == 'feature_action_seq':
                    obs_deque.append(obs)
    ...
    def optimize_off_policy(self, steps, epoch = 0):
        ...
        if steps >= self.initial_rl_steps and steps >= self.initial_srl_steps and self.srl_rl_initial_update_steps>0:
            bar = tqdm(range(self.srl_rl_initial_update_steps))
            for step in bar:
                bar.set_description("INITIAL TRAINING OF SRL AND RL")
                batch = self.memory.sample_srl(self.batchsize, method = self.srl_info['method'])
                batch_rl = self.srl_module.convert_batch(batch.copy())  
                if self.srl_info['use_srl'] and self.srl_info['rl_loss'] and self.srl_info['state_type']=='state':
                    self.rl_info = self.rl_agent.optimize(batch_rl, steps=step, latent_optimizer=self.srl_module.latent_model.optimizer)
                else:
                    self.rl_info = self.rl_agent.optimize(batch_rl, steps=step)
                if self.srl_info['use_srl']: 
                    self.srl_loss_info, srl_info_img = self.srl_module.optimize(batch, step=step)
        ...
        if steps > self.initial_rl_steps and steps % self.param.RL.UPDATE_EVERY == 0: 
            batch_rl = self.srl_module.convert_batch(batch.copy())           
            if self.srl_info['use_srl'] and self.srl_info['rl_loss'] and self.srl_info['state_type']=='state':
                self.rl_info = self.rl_agent.optimize(batch_rl, steps=steps, latent_optimizer=self.srl_module.latent_model.optimizer)
            else:
        ...
        if steps >= self.initial_srl_steps and self.srl_info['use_srl']:
            if self.srl_initial_update_steps>0:
                ...
                if srl_info_img['reconstructed']:
                    self.logger.log_image('observation and reconstruction',torch.cat((srl_info_img['obs_t'], srl_info_img['decoded_t']),0), epoch)  
        ...
        if steps % self.steps_per_epoch == 0 and steps > self.initial_srl_steps and self.srl_info['use_srl']:
            if srl_info_img['reconstructed']:
                self.logger.log_image('observation and reconstruction',torch.cat((srl_info_img['obs_t'], srl_info_img['decoded_t']),0), epoch)  
    ...
    def optimize_on_policy(self, steps, epoch = 0):
        ...
        if steps >= self.initial_srl_steps and self.srl_info['use_srl'] and self.srl_initial_update_steps>0:
            bar = tqdm(range(self.srl_initial_update_steps))
            for _ in bar:
                bar.set_description("INITIAL TRAINING OF SRL")
            ...
            for update_epoch in range(self.on_policy_update_steps):
                ...
                for minibatch in generator:
                    if self.srl_info['use_srl'] and self.joint_training:
                        self.srl_loss_info, srl_info_img = self.srl_module.optimize(minibatch)
                        self.logger.store(**self.srl_loss_info)
                ...
                if self.target_kl is not None and np.mean(approx_kl_divs) > 1.5 * self.target_kl:
                    print(f"Early stopping at step {update_epoch} due to reaching max kl: {np.mean(approx_kl_divs):.2f}")
                    break
    ...
    def load_parameters(self, experiment_name):
        ...
        """
        experiment_parameter_path = os.path.dirname(inspect.getfile(self.__class__)) + '/experiments/' + experiment_name
