...
from srl_framework.srl.losses.losses import kl_divergence_seq, log_liklihood, log_liklihood_from_dist, reconstruction_loss, calculate_kl_divergence
...
class LatentNetwork(nn.Module):
    ...
    def __init__(self, img_channels, state_dim, act_dim, seq_len=8, latent1_dim=32, normalized_obs=False,
                latent2_dim=256, img_size=84, encoder_args={}, gaussian_args={}, lr=0.001):
        ...
        # p(z1(t+1) | z2(t), a(t))
        self.latent1_prior = Gaussian(latent2_dim + act_dim, latent1_dim, **gaussian_args)
        # p(z2(t+1) | z1(t+1), z2(t), a(t))
        self.latent2_prior = Gaussian(latent1_dim + latent2_dim + act_dim, latent2_dim, **gaussian_args)
        ...
        self.latent1_posterior = Gaussian(state_dim + latent2_dim + act_dim, latent1_dim, **gaussian_args)
        # q(z2(t+1) | z1(t+1), z2(t), a(t)) = p(z2(t+1) | z1(t+1), z2(t), a(t))
        self.latent2_posterior = self.latent2_prior

        # p(r(t) | z1(t), z2(t), a(t), z1(t+1), z2(t+1))
        self.reward_predictor = Gaussian(2 * latent1_dim + 2 * latent2_dim + act_dim,1, **gaussian_args)
        ...
        if encoder_args['architecture'] == 'impala':
            ...
            self.decoder = ResNetDecoder(
                feature_dim= latent1_dim + latent2_dim, in_dim=self.encoder.output_dim,
            ...
            self.decoder = PixelDecoder((img_channels,img_size,img_size),latent1_dim + latent2_dim)
        else:
            ...
            self.decoder = CnnDecoder(
                feature_dim=latent1_dim + latent2_dim, in_dim=self.encoder.output_dim, out_dim=img_channels,
    ...
    def sample_prior(self, actions_seq):
        ...
        for t in range(1, actions_seq.size(1) + 1):
            # p(z1(t) | z2(t-1), a(t-1))
            z1_mean, z1_std = self.latent1_prior(torch.cat([z2, actions_seq[:, t - 1]], dim=1))
            z1 = z1_mean + torch.rand_like(z1_std) * z1_std
            # p(z2(t) | z1(t), z2(t-1), a(t-1))
            z2_mean, z2_std = self.latent2_prior(torch.cat([z1, z2, actions_seq[:, t - 1]], dim=1))
            z2 = z2_mean + torch.rand_like(z2_std) * z2_std
    ...
    def sample_posterior(self, features_seq, actions_seq):
        ...
        for t in range(1, actions_seq.size(1) + 1):
            ...
            z1_mean, z1_std = self.latent1_posterior(torch.cat([features_seq[:, t], z2, actions_seq[:, t - 1]], dim=1))
            z1 = z1_mean + torch.rand_like(z1_std) * z1_std
            # q(z2(t) | z1(t), z2(t-1), a(t-1))
            z2_mean, z2_std = self.latent2_posterior(torch.cat([z1, z2, actions_seq[:, t - 1]], dim=1))
            z2 = z2_mean + torch.rand_like(z2_std) * z2_std
    ...
    def calc_latent_loss(self, obs_seq, action_seq, reward_seq, done):
        ...
        if action_seq.ndim <3: action_seq = action_seq.unsqueeze(-1)
        if reward_seq.ndim<3: reward_seq = reward_seq.unsqueeze(-1)
        if done.ndim<3: done = done.unsqueeze(-1)

        # Sample from latent variable model.
        latent1_post_mean, latent1_post_std, latent1_samples, latent2_samples = self.sample_posterior(feature_seq, action_seq)
        latent1_prior_mean, latent1_prior_std = self.sample_prior(action_seq)

        # Calculate KL divergence loss.
        loss_kld = calculate_kl_divergence(latent1_post_mean, latent1_post_std, latent1_prior_mean, latent1_prior_std).mean(dim=0).sum()
