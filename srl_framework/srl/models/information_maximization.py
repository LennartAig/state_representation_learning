...
class DeepInfoMax(BaseModelSRL):
    ...
    def __init__(
        ...
        self.normalized_latent = (
            True
            if params.NORMALIZED_LATENT and not params.CNN.NORMALIZED_LATENT
            else False
        )
        self.squashed_latent = (
            True if params.SQUASHED_LATENT and not params.CNN.SQUASHED_LATENT else False
        )
...
class JSD_ST_DIM_Loss(nn.Module):
    ...
    def forward(
        ...
        feat_local_map_tp1,
        feat_global_tp1,
        feat_local_map_t_hat,
    ):
        sy = feat_local_map_t.size(1)
        sx = feat_local_map_t.size(2)
        feat_global_map_tp1 = (
            feat_global_tp1.unsqueeze(1).unsqueeze(1).expand(-1, sy, sx, self.state_dim)
        )

        target = torch.cat(
            (
                torch.ones_like(feat_global_map_tp1[:, :, :, 0]),
                torch.zeros_like(feat_global_map_tp1[:, :, :, 0]),
            ),
            dim=0,
        ).to(self.device)
...
#####
def InfoNCE_no_action_loss(
    model, s_t, a_t, r_t, s_t_p_1, args, s_t_p_k=None, target=None
):
    device = args["device"]
    ...
    if args["score_fn"] not in (
        "nce_scores_log_softmax",
        "nce_scores_log_softmax_expanded",
    ):
    ...
    if args["lambda_LL"] != 0:
        ...
        nce_L_L, reg_L_L = abstract_scores_action(
            psi_local_LL_t,
            psi_local_LL_t_p_1,
            s_t_local,
            s_t_p_1_local,
    ...
    if args["lambda_LG"] != 0:
        ...
        nce_L_G, reg_L_G = abstract_scores_action(
            psi_local_LG,
            psi_global_LG,
            s_t_local,
            s_t_p_1_global,
    ...
    if args["lambda_GL"] != 0:
        ...
        nce_G_L, reg_G_L = abstract_scores_action(
            psi_global_GL,
            psi_local_GL,
            s_t_global,
            s_t_p_1_local,
    ...
    if args["lambda_GG"] != 0:
        ...
        nce_G_G, reg_G_G = abstract_scores_action(
            psi_global_GG_t,
            psi_global_GG_t_p_1,
            s_t_global,
            s_t_p_1_global,
...
def temporal_DIM_scores(reference, positive, clip_val=20):
    ...
    pairs = clip_val * torch.tanh((1.0 / clip_val) * pairs)
    shape = pairs.shape
