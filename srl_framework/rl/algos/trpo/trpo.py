...
class TRPOAgent(nn.Module):
    ...
    def optimize(self, batch, latent_model_optimizer = None):
        ...
        states, act, adv, log_probs, ret = batch['critic_state_t'], batch['act'], batch['adv'], batch['logp'], batch['ret_gae']
        # Compute Advantages
        ...
        # Policy loss
        explained_variance = (1 - (batch['ret'] - batch['val']).pow(2).sum()/(batch['ret']-batch['ret'].mean()).pow(2).sum()).item()
        ...
        """
        info = dict(explained_variance=0,
                      entropy=0,
                      kl = 0,
