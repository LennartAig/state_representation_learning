...
from srl_framework.rl.algos.sac.core import SquashedGaussianPolicy, DoubleActionValueFunction, weight_init
...
class SACAgent(nn.Module):
    ...
    def __init__(self, encoder=None, polyak=0.995, gamma=0.99,  **kwargs):
        ...
        self.critic = DoubleActionValueFunction(encoder = encoder, **kwargs).to(kwargs['device'])
        print('Critic')
        print(self.critic)
        actor_encoder = deepcopy(self.critic.encoder) if self.critic.use_encoder else None
        self.actor = SquashedGaussianPolicy(encoder = actor_encoder, **kwargs).to(kwargs['device'])
        print('Actor')
        print(self.actor)
        if kwargs['target_encoder']:
            self.critic_target = DoubleActionValueFunction(encoder = kwargs['target_encoder'], **kwargs).to(kwargs['device'])
    ...
    def compute_loss_q(self, batch):
        ...
        if len(act.shape) <= 1: act = act.unsqueeze(-1)
            
        with torch.no_grad():
            # Target actions come from *current* policy
            new_act_tp1, log_p_tp1 = self.actor(actor_state_tp1)
            # Target Q-Values
            q1_target_val, q2_target_val = self.critic_target(critic_target_state_tp1, new_act_tp1)
            if log_p_tp1.ndim > q1_target_val.ndim:
                log_p_tp1 = log_p_tp1.squeeze(-1)
            q_target_val = torch.min(q1_target_val, q2_target_val) - self.alpha.detach() * log_p_tp1
            ...
            if self.drq:
                ...
                # Target Q-Values
                q1_target_val_aug, q2_target_val_aug = self.critic_target(critic_target_state_tp1_aug, new_act_tp1_aug)
                q_target_val_aug = torch.min(q1_target_val_aug, q2_target_val_aug) - self.alpha.detach() * log_p_tp1
        ...
        q_info = dict(Q1Vals=q1_val.detach().cpu().numpy(),
    ...
    def compute_loss_pi(self, batch, detach=True):
        ...
        alpha_loss = (self.alpha *
                        (-log_p - self.target_entropy).detach()).mean()
        
        pi_info = dict(LogPi=log_p.detach().cpu().numpy(),
                       LossPi = actor_loss.detach().cpu().numpy())
    ...
    def optimize(self, batch, steps=0, latent_optimizer = None):
        ...
        if latent_optimizer: latent_optimizer.zero_grad()
        self.critic_optimizer.zero_grad()
        loss_q.backward()
        self.critic_optimizer.step()
        if latent_optimizer: latent_optimizer.step()
