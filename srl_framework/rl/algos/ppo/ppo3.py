...
class PPOAgent2(nn.Module):
    ...
    def get_loss_info_dict(self):
        return dict(
            LossPi=0, LossV=0, KL=0, Entropy=0, ClipFrac=0, DeltaLossPi=0, DeltaLossV=0
        )
    ...
    def optimize(self, batch, latent_optimizer=None):
        ...
        pi_l_old, _, pi_info_old = self.compute_loss_pi(
            batch["actor_state_t"], batch["act"], batch["adv"], batch["logp"]
        )
        v_l_old = self.compute_loss_v(
            batch["critic_state_t"], batch["ret"], batch["val"]
        )
