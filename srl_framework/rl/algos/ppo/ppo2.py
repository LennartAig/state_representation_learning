...
class PPOAgent2(nn.Module):
    ...
    def __init__(self, encoder=None, **kwargs):
        ...
        if kwargs['lr_scheduler']:
            ...
            self.critic_scheduler = optim.lr_scheduler.LambdaLR(self.critic_optimizer, schedule) 
        self.actor.train()
        self.critic.train()
    ...
    def compute_loss_pi(self, obs, act, adv, logp_old):
        ...
        loss_pi += self.cutoff_coef * (kl_div > 2 * self.max_kl_div) * (kl_div - self.max_kl_div)**2
    ...
    def optimize(self, batch, latent_optimizer = None,num_minibatch=0):       
        loss_v = self.compute_loss_v(batch['critic_state_t'], batch['ret_gae'], batch['val'])
