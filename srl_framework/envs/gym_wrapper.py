...
class GymWrapper(Wrapper):
    ...
    def __init__(self,
        ...
        if action_norm:
            assert (action_norm and isinstance(env.action_space, spaces.Box)), (
                "Action norm set. Expected Box action space, got {}".format(type(env.action_space)))
            self.a = np.zeros(env.action_space.shape, dtype=env.action_space.dtype) -1.0
            self.b = np.zeros(env.action_space.shape, dtype=env.action_space.dtype) +1.0
            self.action_space = spaces.Box(low=-1.0, high=1.0, shape=env.action_space.shape, dtype=env.action_space.dtype)
        ...
        if obs_type == 'state':
            ...
            if channel_first:
                image_shape = (3, self._render_kwargs['height'], self._render_kwargs['width'])
            else:
                image_shape = (self._render_kwargs['height'], self._render_kwargs['width'], 3)
