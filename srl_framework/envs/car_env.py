...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, width=64, height=64, verbose=0):
        ...
        # channel first
        self.observation_space = spaces.Box(low=0, high=255, shape=(3, self.state_height, self.state_width), dtype=np.uint8)
    ...
    def render(self, mode='human'):
        ...
        if mode == 'rgb_array':
            ...
            VP_W = self.state_width
            VP_H = self.state_height
        else:
            pixel_scale = 1
        ...
        if mode == 'state_pixels':
            # channel first, just like pytorch likes it
            # arr = np.einsum('ijk->kij',arr).copy()
