...
class Buffer:
    ...
    def __init__(self, capacity, obs_shape, act_dim, device, **kwargs):
        ...
        if self.mini_batch_size:
            ...
            self.batch_sampler = BatchSampler(
                random_sampler, self.mini_batch_size, drop_last=True
            )
        if self.drq:
    ...
    def finish_path(self, last_value=0, maximum_entropy=False):
        ...
        # Maximum Entropy
        if maximum_entropy:
            rewards -= self.tau * self.log_prob[idxs_episode]
    ...
    def sample(self, batch_size):
        ...
        assert (
            self.ptr - self.epoch_ptr
        ) == self.steps_per_epoch  # check if reset was successfull
        start = self.epoch_ptr
        adv = torch.as_tensor(
            self.advantage[start : self.ptr], dtype=torch.float32, device=self.device
        )
        ...
        if self.drq:
            ...
            obs_tp1_aug = torch.as_tensor(
                obs_tp1_aug, dtype=torch.float32, device=self.device
            )
        ...
        return dict(
            ...
            act=torch.as_tensor(
                self.act[start : self.ptr], dtype=torch.float32, device=self.device
            ),
            ret=torch.as_tensor(
                self.return_mc[start : self.ptr],
                dtype=torch.float32,
                device=self.device,
            ),
            ret_gae=torch.as_tensor(
                self.return_gae[start : self.ptr],
                dtype=torch.float32,
                device=self.device,
            ),
            adv=adv,
            val=torch.as_tensor(
                self.val[start : self.ptr], dtype=torch.float32, device=self.device
            ),
            logp=torch.as_tensor(
                self.log_prob[start : self.ptr], dtype=torch.float32, device=self.device
            ),
        )
    ...
    def sample_srl(self, batch_size, method="none", idxs=None):
        idxs = (
            np.random.randint(0, self.size, size=batch_size) if idxs == None else idxs
        )
        obs = self.obs[idxs]
        ...
        return dict(
            ...
            obs_tp1_aug=obs_tp1_aug,
            act=torch.as_tensor(
                self.act[idxs], dtype=torch.float32, device=self.device
            ),
            rew=torch.as_tensor(
                self.rew[idxs], dtype=torch.float32, device=self.device
            ),
    ...
    def generator(self, rollouts, converted_batch=False):
        if self.mini_batch_size:
            for indices in self.batch_sampler:
                ...
                if self.drq:
                    minibatch["obs_aug"] = rollouts["obs_aug"][indices]
                if self.drq:
                    minibatch["obs_tp1_aug"] = rollouts["obs_tp1_aug"][indices]
                ...
                if converted_batch:
                    minibatch["critic_state_tp1"] = rollouts["critic_state_tp1"][
                        indices
                    ]
        ...
        if self.drq:
            ...
            return dict(
                ...
                rew=torch.as_tensor(
                    ...
                    self.done[idxs], dtype=torch.int, device=self.device
                ),
            )
        else:
            return dict(
                obs=torch.as_tensor(obs, dtype=torch.float32, device=self.device),
                obs_tp1=torch.as_tensor(
                    obs_tp1, dtype=torch.float32, device=self.device
                ),
                act=torch.as_tensor(
                    self.act[idxs], dtype=torch.float32, device=self.device
                ),
...
class SequentialReplayBuffer:
    ...
    def __init__(self, capacity, obs_shape, act_dim, device, **kwargs):
        ...
        # Transition data
        self.obs = np.zeros(
            combined_shape(self.capacity, combined_shape(self.ep_len, obs_shape)),
            dtype=obs_dtype,
        )
        ...
        self.act = np.zeros(
            combined_shape(self.capacity, combined_shape(self.ep_len, act_dim)),
            dtype=np.float32,
        )
        self.act_tm1 = np.zeros(
            combined_shape(self.capacity, combined_shape(self.ep_len + 1, act_dim)),
            dtype=np.float32,
        )
        self.rew = np.zeros(
            combined_shape(self.capacity, self.ep_len), dtype=np.float32
        )
        self.done = np.zeros(
            combined_shape(self.capacity, self.ep_len), dtype=np.float32
        )
    ...
    def store(self, obs, act, rew, obs_tp1, done, hidden_in=0, hidden_out=0):
        ...
        self.act_tm1[self.ptr][
            self.ep_ptr + 1
        ] = act  # memory expensive but saves computation
    ...
    def update_index(self, batch_size=32, random=True):
        self.index = (
            min(self.index + batch_size, self.size) % self.size
            if self.sub_index + self.sequential_len >= self.seq_len[self.index]
            else self.index
        )
        self.sub_index = (
            0
            if random
            else min(self.sub_index + self.sequential_len, self.seq_len[self.index])
            % self.seq_len[self.index]
        )
    ...
    def sample(self, batch_size=32, random=True, sequential=True, method="none"):
        ...
        if random:
            idxs = np.array(
                [i for i, v in enumerate(self.seq_len >= self.sequential_len + 5) if v]
            )
            ...
            sub_idx = [
                np.random.randint(
                    0,
                    min(
                        self.ep_len - self.sequential_len,
                        self.seq_len[i] - self.sequential_len,
                    ),
                )
                for i in idxs
            ]
        ...
        if sequential:
            sub_idx_range = [
                range(sub_i, min(sub_i + self.sequential_len, self.seq_len[idxs[i]]))
                for i, sub_i in enumerate(sub_idx)
            ]
        ...
        if method == "slac":
            ...
            obs_tm8_tm1 = np.array(
                [self.obs[idxs[i]][sub_idx_range[i]] for i in range(batch_size)]
            )
            obs_t = np.expand_dims(
                np.array(
                    [
                        self.obs_tp1[idxs[i]][sub_idx_range[i][-1]]
            ...
            act = np.array(
                [self.act[idxs[i]][sub_idx_range[i]] for i in range(batch_size)]
            )
            test = 1
        else:
            obs = np.array(
        ...
        if self.safe_hidden:
            batch = dict(
                hidden_in=(
                    np.array(
                        [
                            self.hidden_in[0][idxs[i]][sub_idx[i]]
                            for i in range(batch_size)
                        ]
                    ),
                    np.array(
                        [
                            self.hidden_in[1][idxs[i]][sub_idx[i]]
                            for i in range(batch_size)
                        ]
                    ),
                ),
                hidden_out=(
                    np.array(
                        [
                            self.hidden_out[0][idxs[i]][sub_idx[i]]
                            for i in range(batch_size)
                        ]
                    ),
                    np.array(
                        [
                            self.hidden_out[1][idxs[i]][sub_idx[i]]
                            for i in range(batch_size)
                        ]
                    ),
                ),
                obs=obs,
                obs_tp1=np.array(
                    [self.obs_tp1[idxs[i]][sub_idx_range[i]] for i in range(batch_size)]
                ),
                act=np.array(
                    [self.act[idxs[i]][sub_idx_range[i]] for i in range(batch_size)]
                ),
                act_tm1=np.array(
                    [self.act_tm1[idxs[i]][sub_idx_range[i]] for i in range(batch_size)]
                ),
                rew=np.array(
                    [self.rew[idxs[i]][sub_idx_range[i]] for i in range(batch_size)]
                ),
                done=np.array(
                    [self.done[idxs[i]][sub_idx_range[i]] for i in range(batch_size)]
                ),
            )
        else:
            batch = dict(
        ...
        return {
            k: torch.as_tensor(v, dtype=torch.float32, device=torch.device(self.device))
    ...
    def sample_slac(self, batch_size=32, random=True, sequential=True, slac=True):
        if random:
            size_temp = (
                self.size - 2
                if self.seq_len[self.size - 1] < (self.sequential_len + 5)
                else self.size - 1
            )
        ...
        obs_t = np.expand_dims(
            np.array(
                [self.obs_tp1[idxs[i]][sub_idx_range[i][-1]] for i in range(batch_size)]
            ),
            axis=1,
        )
        obs = np.concatenate((obs_tm8_tm1, obs_t), axis=1)
