...
class CnnDecoder(nn.Module):
    ...
    -
    """
    def __init__(self, feature_dim=128, in_dim=31, out_dim =3, architecture='std',
                activation="ReLU", batchnorm=False, pool=False,
        ...
        self.input_layer = nn.Linear(feature_dim, self.filter_size * in_dim * in_dim).apply(inits[linear_init])

        self.decoder = make_decoder(channels=channels, kernels=kernel_sizes, strides=stride_size,
            paddings=padding_sizes, activation=activation, batchnorm=batchnorm, pool=pool,
            dropout = dropout, conv_init=conv_init)
...
class ResNetDecoder(nn.Module):
    def __init__(self, feature_dim=128, in_dim=21, out_dim =9, img_size=84):
        ...
        if img_size==128 or img_size==64:
            ...
            raise NotImplementedError
        self.layer1 = self._make_layer(self.depths[0], self.depths[1], kernels[0],paddings[0],output_paddings[0])
        self.layer2 = self._make_layer(self.depths[1], self.depths[2], kernels[1],paddings[1],output_paddings[1])
        self.layer3 = self._make_layer(self.depths[2], self.depths[3], kernels[2],paddings[2],output_paddings[2])
        self.layer4 = self._make_layer(self.depths[3], out_dim, kernels[2],paddings[2],output_paddings[2])
        self.flatten = Flatten()
        self.train()

    def _make_layer(self, in_channels, depth, kernelsize= 3, padding=0, output_padding=0):
        return nn.Sequential(
            nn.ConvTranspose2d(in_channels, depth, kernelsize, stride=2, output_padding=output_padding, padding=padding),
