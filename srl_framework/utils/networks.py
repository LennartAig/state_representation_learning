...
def make_mlp(
    ...
    layers = [
        mlp_layer(
            in_, out_, activation_=activation, batchnorm=batchnorm, dropout=dropout
        )
        for in_, out_ in zip(layer_sizes[:-2], layer_sizes[1:-1])
    ]
    if output_layer:
...
def make_cnn(
    ...
    layers = []
    for in_, out_, ker_, stride_, pad_ in zip(
        channels[:-1], channels[1:], kernels, strides, paddings
    ):
        layers.append(
            conv_layer(
                in_,
...
def make_decoder(
    ...
    layers = []
    for in_, out_, ker_, stride_, pad_ in zip(
        channels[:-2], channels[1:-1], kernels[:-1], strides[:-1], paddings[:-1]
    ):
        layers.append(
            deconv_layer(
                in_,
...
def conv_layer(
    ...
    l = nn.ModuleList(
        [
            nn.Conv2d(
                in_, out_, kernel_size=ker_, stride=stride_, padding=pad_, bias=bias
            )
        ]
    )
    if batchnorm:
...
def naive(m):
    if isinstance(m, nn.Linear):
        ...
        nn.init.uniform_(
            m.weight,
            a=-math.sqrt(1.0 / float(fan_in)),
            b=math.sqrt(1.0 / float(fan_in)),
        )
