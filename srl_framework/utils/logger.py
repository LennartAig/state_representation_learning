...
class Logger:
    ...
    def __init__(self, log_dir=None, param_path = '../experiment.yaml', output_fname='progress.txt', exp_name=None, normalized_image=False,args=dict()):
        """
        Initialize a Logger.
        ...
        if osp.exists(log_dir):
            print("Warning: Log dir %s already exists! Storing info there anyway."%log_dir)
        ...
        print(colorize("Tensorboard is logged to %s"%self.log_dir, 'green', bold=True))
    ...
    def _try_sw_log_image(self, key, image, step):
        if self.writer is not None:
            #assert image.dim() == 3
            #if image.shape[0] > 3:
            images = torch.zeros([int(image.shape[0]/3),3,image.shape[1],image.shape[2]], dtype=torch.float32)
    ...
    def plot_grad_flow(self, named_parameters, epoch):
        ...
        fig.savefig(self.fig_dir + 'gradient_flow_epoch{}.png'.format(epoch), bbox_inches='tight', dpi=150)
...
def create_epoch_logger(param, parameter_path,args):
    ...
    log_dir =  os.path.join(fileDir, '../../log',obs_type, env_type, env_name, rl_name, srl_name, timestamp)
    return EpochLogger(log_dir = log_dir, param_path = parameter_path, normalized_image= args.normalize_obs, args=args)