...
class CnnEncoder(nn.Module):
    ...
    def __init__(
        ...
        if architecture == "std":
            ...
            assert (
                img_size >= 84
            ), "Image Size must be at least 128, should be higher with nature cnn!"
            OUT_DIM = {128: 14, 84: 7}
    ...
    def copy_conv_weights_from(self, source):
        i = 0
        for module_src, module_targ in zip(
            source.encoder.modules(), self.encoder.modules()
        ):
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(img_channels, self.depths[0]).apply(
            inits[conv_init]
        )
        self.layer2 = self._make_layer(self.depths[0], self.depths[1]).apply(
            inits[conv_init]
        )
        self.layer3 = self._make_layer(self.depths[1], self.depths[2]).apply(
            inits[conv_init]
        )
        self.layer4 = self._make_layer(self.depths[2], self.depths[3]).apply(
            inits[conv_init]
        )

        self.out_dim = OUT_DIM[img_size]
        self.final_conv_size = self.depths[-1] * self.out_dim * self.out_dim
        self.output_layer = nn.Linear(self.final_conv_size, feature_dim).apply(
            inits[linear_init]
        )
        self.flatten = Flatten()
        if self.normalize:
